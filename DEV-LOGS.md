...
- What:
  ...
  - removed deva-bridge-tmux and deva-bridge-tmux-host
  ...
  - OPT-IN pivot (design review): host-tmux is a host-exec escape hatch, so it is NOT baked into the image (would hand every agent host command execution — deva mounts host ~/.ssh, auto-mounts docker.sock; either is a turnkey escape once host-tmux is present). Removed the Dockerfile/Dockerfile.rust COPY. Added `deva.sh --host-tmux`: mounts scripts/host-tmux + host ~/.ssh (ro) on request, dedups the ssh mount against a user .deva that already mounts it (added after user volumes, before validate_bind_mount_shape). New helper docker_args_has_mount_target. Tests: 3 cases in scripts/test-mount-shape.sh (off / on-no-ssh / on-with-user-ssh).
  - docs/tmux-bridge-agent-comms.md: reframed around the sandbox. Safe default is host->container (deva.sh shell / docker exec — host reaches in, container gains nothing). tmux-bridge Layer 2 stays sandboxed. host-tmux is the opt-in escape hatch; --no-docker config stays sealed unless --host-tmux is passed.
- Result: sandbox intact by default; host reach is explicit per-run opt-in. Verified: host-tmux live against OrbStack/macOS (doctor/setup/quoting/tty-guard/bridge/Layer-2); deva.sh --host-tmux via dry-run + full scripts/test-mount-shape.sh suite (all pass). NOT verified from here: a real `--host-tmux` container run end-to-end (needs image build/run outside this container) — dry-run confirms the docker argv only
