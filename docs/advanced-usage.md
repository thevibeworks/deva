...
(cctrace >= 0.11) and always run MITM capture. opencode, pi, dsh, and
cursor have no cctrace profile yet, so `--trace` on those agents is
rejected (thevibeworks/cctrace#89).
