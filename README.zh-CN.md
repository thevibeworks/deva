...
在 Docker 里跑 Claude Code、Codex、Gemini、Grok、Kimi、opencode、pi 七个 agent CLI，并且不再假装 agent 自带的权限弹窗是在保护你。

容器即沙箱，挂载即契约。这换来的是：

- 全速跑 agent。权限弹窗存在的理由是爆炸半径等于你的整机；把爆炸半径缩小到一个容器，YOLO 就不再是鲁莽 —— 七个 agent 全部关掉自带权限系统跑（`claude --dangerously-skip-permissions`、`codex --dangerously-bypass-approvals-and-sandbox`、`gemini --yolo`、`grok --always-approve`、`kimi --yolo`、opencode 走 `OPENCODE_PERMISSION` 全放行、pi 本身就没有权限系统）。最坏结果随容器一起销毁。
